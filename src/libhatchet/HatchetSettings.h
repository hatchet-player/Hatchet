...
#ifndef HATCHET_SETTINGS_H
#define HATCHET_SETTINGS_H
...
#define HATCHET_SETTINGS_VERSION 17

/**
 * Convenience wrapper around QSettings for hatchet-specific config
 */
class DLLEXPORT HatchetSettings : public QSettings
{
Q_OBJECT

public:
    static HatchetSettings* instance();

    explicit HatchetSettings( QObject* parent = 0 );
    virtual ~HatchetSettings();
    ...
    QStringList scannerPaths() const; /// QDesktopServices::MusicLocation in HatchetSettingsGui
    ...
    Hatchet::PlaylistModes::RepeatMode repeatMode( const QString& playlistid );
    void setRepeatMode( const QString& playlistid, Hatchet::PlaylistModes::RepeatMode mode );
    ...
    /// Network settings
    Hatchet::Network::ExternalAddress::Mode externalAddressMode();
    void setExternalAddressMode( Hatchet::Network::ExternalAddress::Mode externalAddressMode );
    ...
    Hatchet::SerializedUpdaters playlistUpdaters() const;
    void setPlaylistUpdaters( const Hatchet::SerializedUpdaters& updaters );
...
private:
    ...
    static HatchetSettings* s_instance;
};

Q_DECLARE_METATYPE( HatchetSettings::PrivateListeningMode )
