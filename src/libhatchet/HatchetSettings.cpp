...
HatchetSettings::doUpgrade( int oldVersion, int newVersion )
{
    ...
    else if ( oldVersion == 4 || oldVersion == 5 )
    {
        ...
        Hatchet::DatabaseFuzzyIndex::wipeIndex();
        updateIndex();
    }
    ...
    else if ( oldVersion == 7 )
    {
        ...
        {
            if ( account.startsWith( "resolveraccount_" ) && value( QString( "%1/accountfriendlyname" ).arg( account ) ).toString().endsWith( "spotify_hatchetresolver" ) )
            {
...
//        qDebug() << "Got accounts to migrate:" << accounts;
    ...
    else if ( oldVersion == 13 )
    {
        ...
        QFile dataFile( HatchetUtils::appDataDir().absoluteFilePath( "echonest_stylesandmoods.dat" ) );
        const bool removed = dataFile.remove();
        tDebug() << "Tried to remove echonest_stylesandmoods.dat, succeeded?" << removed;
    }
    else if ( oldVersion == 14 )
    {
        //No upgrade on OSX: we keep storing credentials in HatchetSettings
        ...
        foreach ( const QString& account, accounts )
        {
            ...
            if ( !creds.isEmpty() )
            {
                QKeychain::WritePasswordJob* j = new QKeychain::WritePasswordJob( QLatin1String( "Hatchet" ), this );
                j->setKey( account );
...
#endif
                bool ok;
                QByteArray data = HatchetUtils::toJson( creds, &ok );
