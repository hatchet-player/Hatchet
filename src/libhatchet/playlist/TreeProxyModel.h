...
public:
    ...
    QModelIndex indexFromArtist( const Hatchet::artist_ptr& artist ) const;
    QModelIndex indexFromAlbum( const Hatchet::album_ptr& album ) const;
    QModelIndex indexFromResult( const Hatchet::result_ptr& result ) const;
    QModelIndex indexFromQuery( const Hatchet::query_ptr& query ) const;
...
protected:
    ...
    bool lessThan( int, const Hatchet::query_ptr&, const Hatchet::query_ptr& ) const {  Q_ASSERT( false ); HatchetUtils::crash(); return false; }

private slots:
    void onRowsInserted( const QModelIndex& parent, int start, int end );

    void onFilterArtists( const QList<Hatchet::artist_ptr>& artists );
    void onFilterAlbums( const QList<Hatchet::album_ptr>& albums );
...
private:
    ...
    mutable QMap< QPersistentModelIndex, Hatchet::query_ptr > m_cache;

    QList<Hatchet::artist_ptr> m_artistsFilter;
    QList<Hatchet::album_ptr> m_albumsFilter;
    Hatchet::ArtistsRequest* m_artistsFilterCmd;
