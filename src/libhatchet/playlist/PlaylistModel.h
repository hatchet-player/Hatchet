...
public:
    ...
    Hatchet::playlist_ptr playlist() const;

    virtual void loadPlaylist( const Hatchet::playlist_ptr& playlist, bool loadEntries = true );
...
public slots:
    ...
    virtual void appendEntries( const QList< Hatchet::plentry_ptr >& entries );

    virtual void insertAlbums( const QList< Hatchet::album_ptr >& album, int row = 0 );
    virtual void insertAlbums( const Hatchet::collection_ptr&, int row = 0 ) { Q_UNUSED( row ); }
    virtual void insertArtists( const QList< Hatchet::artist_ptr >& artist, int row = 0 );
...
protected:
    ...
    QList<Hatchet::plentry_ptr> playlistEntries() const;

private slots:
    void onRevisionLoaded( Hatchet::PlaylistRevision revision );
    void parsedDroppedTracks( QList<Hatchet::query_ptr> );
