...
ColumnItemDelegate::sizeHint( const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
    ...
    if ( index.isValid() )
    {
        Hatchet::ModelTypes type = (Hatchet::ModelTypes)index.data( PlayableProxyModel::TypeRole ).toInt();
        switch ( type )
        {
            case Hatchet::TypeAlbum:
            {
                size.setHeight( option.fontMetrics.height() * 5 );
                return size;
            }

            case Hatchet::TypeQuery:
            case Hatchet::TypeResult:
            {
...
ColumnItemDelegate::paint( QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
    ...
    else if ( !item->result().isNull() || !item->query().isNull() )
    {
        ...
        QColor textColor = HatchetUtils::alphaBlend( option.palette.color( QPalette::Foreground ), option.palette.color( QPalette::Background ), opacity );

        {
            ...
            if ( item->isPlaying() )
            {
                textColor = HatchetStyle::NOW_PLAYING_ITEM_TEXT;
                o.palette.setColor( QPalette::Highlight, HatchetStyle::NOW_PLAYING_ITEM );
                o.palette.setColor( QPalette::Text, HatchetStyle::NOW_PLAYING_ITEM_TEXT );
                o.state |= QStyle::State_Selected;
            }
...
/*            if ( m_hoveringOver == index && !index.data().toString().isEmpty() && index.column() == 0 )
            {
                ...
                QPixmap infoIcon = HatchetUtils::defaultPixmap( HatchetUtils::InfoIcon, HatchetUtils::Original, arrowRect.size() );
...
//    painter->drawPixmap( r, QPixmap( RESPATH "images/cover-shadow.png" ) );
    ...
    if ( !m_pixmaps.contains( index ) )
    {
        ...
        {
            m_pixmaps.insert( index, QSharedPointer< Hatchet::PixmapDelegateFader >( new Hatchet::PixmapDelegateFader( item->album(), r.size(), HatchetUtils::Original, false ) ) );
        ...
        {
            m_pixmaps.insert( index, QSharedPointer< Hatchet::PixmapDelegateFader >( new Hatchet::PixmapDelegateFader( item->artist(), r.size(), HatchetUtils::Original, false ) ) );
            _detail::Closure* closure = NewClosure( m_pixmaps[ index ], SIGNAL( repaintRequest() ), const_cast<ColumnItemDelegate*>(this), SLOT( doUpdateIndex( const QPersistentModelIndex& ) ), QPersistentModelIndex( index ) );
            closure->setAutoDelete( false );
        }
    }
