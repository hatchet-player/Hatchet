...
Cache::Cache()
    : QObject( 0 )
    , m_cacheBaseDir( HatchetSettings::instance()->storageCacheLocation() + "/GenericCache/" )
    , m_cacheManifest( m_cacheBaseDir + "cachemanifest.ini", QSettings::IniFormat )
{
    if ( HatchetSettings::instance()->genericCacheVersion() < s_cacheVersion )
    {
        HatchetUtils::removeDirectory( m_cacheBaseDir );
        HatchetSettings::instance()->setGenericCacheVersion( s_cacheVersion );
    }
...
Cache::pruneTimerFired()
{
    ...
    qDebug() << Q_FUNC_INFO << "Pruning hatchetcache";
    ...
    foreach ( const QVariant& client, clients )
    {
        ...
        {
            CacheData data = cached_settings.value( key ).value<HatchetUtils::CacheData>();
