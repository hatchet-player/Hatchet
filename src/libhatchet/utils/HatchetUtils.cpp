...
appFriendlyVersion()
{
    QStringList l = QString( HATCHET_VERSION ).split( ".", QString::SkipEmptyParts );
    while ( l.count() > 3 )
        l.removeLast();
...
appConfigDir()
{
    ...
    {
        ret = QDir( QString( "%1/" HATCHET_APPLICATION_NAME ).arg( getenv( "XDG_CONFIG_HOME" ) ) );
    }
    else if ( getenv( "HOME" ) )
    {
        ret = QDir( QString( "%1/.config/" HATCHET_APPLICATION_NAME ).arg( getenv( "HOME" ) ) );
    }
    else
    {
...
logFilePath()
{
    return HatchetUtils::appLogDir().filePath( HATCHET_APPLICATION_NAME ".log" );
}
...
mergePlaylistChanges( const QList< Hatchet::query_ptr >& orig, const QList< Hatchet::query_ptr >& newTracks, bool& changed )
{
    int sameCount = 0;
    QList< Hatchet::query_ptr > tosave = newTracks;
    changed = false;

    foreach ( const Hatchet::query_ptr& newquery, newTracks )
    {
        foreach ( const Hatchet::query_ptr& oldq, orig )
        {
...
class HatchetTranslator : public QTranslator
{
public:
    HatchetTranslator( QObject* parent ) : QTranslator( parent )
    {
    };
    ...
    QString translate( const char * context, const char * sourceText, const char * disambiguation = 0, int n = -1) const Q_DECL_OVERRIDE
    {
        ...
        return translation.replace( "%applicationName", HATCHET_APPLICATION_NAME, Qt::CaseInsensitive );
    }
};
...
installTranslator( QObject* parent )
{
    ...
    // Hatchet translations
    QTranslator* translator = new HatchetTranslator( parent );
    if ( translator->load( QString( ":/lang/hatchet_" ) + locale ) )
    {
        qDebug() << "Translation: Hatchet: Using system locale:" << locale;
    }
    else
    {
        qDebug() << "Translation: Hatchet: Using default locale, system locale one not found:" << locale;
        translator->load( QString( ":/lang/hatchet_en" ) );
    }
...
verifyFile( const QString& filePath, const QString& signature )
{
    ...
    // Public key is in :/data/misc/hatchet_pubkey.pem
    QFile f( ":/data/misc/hatchet_pubkey.pem" );
    ...
    {
        qWarning() << "Loaded Hatchet public key but cannot use it to verify! What is up....";
        return false;
    }
...
{
    // uses QuaZip to extract the temporary zip file to the user's hatchet data/resolvers directory
    QDir resolverDir = appDataDir();
    if ( !resolverDir.mkpath( QString( "%1/%2" ).arg( dirName )
                                                .arg( resolverId ) ) )
    {
        tLog() << "Failed to mkdir resolver save dir:" << HatchetUtils::appDataDir().absoluteFilePath( QString( "%1/%2" ).arg( dirName ).arg( resolverId ) );
        return QString();
    }
