...
{
    qRegisterMetaType< QHash< Hatchet::playlist_ptr, QStringList > >("QHash< Hatchet::playlist_ptr, QStringList >");
}
...
DatabaseCommand_LoadAllPlaylists::exec( DatabaseImpl* dbi )
{
    ...
    while ( query.next() )
    {
        ...
        if ( d->returnPlEntryIds )
        {
            QStringList trackIds = HatchetUtils::parseJson( query.value( 8 ).toByteArray() ).toStringList();
