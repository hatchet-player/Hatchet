...
MusicScanner::readTags( const QFileInfo& fi )
{
    ...
    if ( !HatchetUtils::supportedExtensions().contains( suffix ) )
    ...
    const QString mimetype = HatchetUtils::extensionToMimetype( suffix );
