...
#ifndef HATCHET_ACLREGISTRY_H
#define HATCHET_ACLREGISTRY_H
...
public:
    ...
    struct User {
        ...
        User()
            ...
            , knownDbids()
            , knownAccountIds()
            , acl( Hatchet::ACLStatus::NotFound )
            {}

        ~User()
            {}

        User( QString p_uuid, QString p_friendlyName, QStringList p_knownDbids, QStringList p_knownAccountIds, Hatchet::ACLStatus::Type p_acl )
            : uuid( p_uuid )
     ...
     */
    virtual void isAuthorizedRequest( const Hatchet::Network::ACL::aclrequest_ptr& request );

signals:
    void aclResult( QString nodeid, QString username, Hatchet::ACLStatus::Type peerStatus );
...
public slots:
     ...
     **/
    virtual Hatchet::ACLStatus::Type isAuthorizedUser( const QString &dbid, const QString &username, Hatchet::ACLStatus::Type globalType = Hatchet::ACLStatus::NotFound, bool skipEmission = false ) = 0;
...
private slots:
    void aclResultForRequest( QString nodeid, QString username, Hatchet::ACLStatus::Type peerStatus );

private:
    Hatchet::Utils::WeakObjectList<Hatchet::Network::ACL::AclRequest> m_aclRequests;
};

Q_DECLARE_METATYPE( ACLRegistry::User )

#endif // HATCHET_ACLREGISTRY_H
