    ...
    : d_ptr( new SipStatusMessagePrivate( this, statusMessageType, contactId, message ) )
{
    ...
    {
        HatchetUtils::ImageType imageType;
        switch( d->statusMessageType )
        {
            case SipLoginFailure:
            case SipInviteFailure:
                imageType = HatchetUtils::ProcessStop;
                break;

            case SipInviteSuccess:
            case SipAuthReceived:
            default:
                imageType = HatchetUtils::AddContact;
        }
        SipStatusMessagePrivate::s_typesPixmaps.insert( d->statusMessageType, HatchetUtils::defaultPixmap( imageType, HatchetUtils::Original, QSize( 64, 64 ) ) );
    }
}
