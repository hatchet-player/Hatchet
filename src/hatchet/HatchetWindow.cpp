...
HatchetWindow::HatchetWindow( QWidget* parent )
    : QMainWindow( parent )
    , HatchetUtils::DpiScaler( this )
    , ui( new Ui::HatchetWindow )
    , m_searchWidget( 0 )
    , m_trayIcon( 0 )
{
#ifndef Q_OS_MAC
    ...
    HatchetUtils::unmarginLayout( ui->centralWidget->layout() );

    if ( QSystemTrayIcon::isSystemTrayAvailable() )
    {
        m_trayIcon = new HatchetTrayIcon( this );
    }
    ...
    if ( HatchetSettings::instance()->fullscreenEnabled() )
    {
...
HatchetWindow::loadSettings()
{
    ...
    ActionCollection::instance()->getAction( "showOfflineSources" )->setChecked( HatchetSettings::instance()->showOfflineSources() );
}


void
HatchetWindow::saveSettings()
{
    HatchetSettings* s = HatchetSettings::instance();
...
void
HatchetWindow::applyPlatformTweaks()
{
...
void
HatchetWindow::setupToolBar()
{
#ifdef Q_OS_MAC
    Hatchet::setupToolBarMac( this );
...
#else
    m_toolbar = addToolBar( "HatchetToolbar" );
    m_toolbar->setObjectName( "HatchetToolbar" );
...
void
HatchetWindow::setupUpdateCheck()
{
    ...
    if ( qApp->arguments().contains( "--debug" ) )
        updaterUrl.setUrl( "http://download.hatchet-player.org/sparklewin-debug" );
    else
        updaterUrl.setUrl( "http://download.hatchet-player.org/sparklewin" );

    qtsparkle::Updater* updater = new qtsparkle::Updater( updaterUrl, this );
    Q_ASSERT( Hatchet::Utils::nam() != 0 );
    updater->SetNetworkAccessManager( Hatchet::Utils::nam() );
    updater->SetVersion( HatchetUtils::appFriendlyVersion() );
...
#ifdef Q_OS_WIN
void
HatchetWindow::setupWindowsButtons()
{
    ...
    back->setIcon( thumbIcon( HatchetUtils::PrevButton ) );
    ...
    play->setIcon( thumbIcon( HatchetUtils::PlayButton ) );
    ...
    next->setIcon( thumbIcon( HatchetUtils::NextButton ) );
    ...
    love->setIcon( thumbIcon( HatchetUtils::NotLoved ) );
...
HatchetWindow::updatePreview()
{
    ...
    if ( cover.isNull() ) {
        cover = HatchetUtils::defaultPixmap( HatchetUtils::DefaultAlbumCover , HatchetUtils::Original, coverSize );
    }
...
void
HatchetWindow::updateWindowsLoveButton()
{
    ...
    if ( !AudioEngine::instance()->currentTrack().isNull() )
    {
        ...
        {
            love->setIcon(thumbIcon(HatchetUtils::Loved));
...
QIcon
HatchetWindow::thumbIcon(HatchetUtils::ImageType type)
{
    return  HatchetUtils::defaultPixmap( type , HatchetUtils::Original, QSize( 40, 40 ) );
}
...
void
HatchetWindow::setupSignals()
{
    // <From AudioEngine>
    connect( AudioEngine::instance(), SIGNAL( error( AudioEngine::AudioErrorCode ) ), SLOT( onAudioEngineError( AudioEngine::AudioErrorCode ) ) );
    connect( AudioEngine::instance(), SIGNAL( loading( const Hatchet::result_ptr& ) ), SLOT( onPlaybackLoading( const Hatchet::result_ptr& ) ) );
    connect( AudioEngine::instance(), SIGNAL( started( Hatchet::result_ptr ) ), SLOT( audioStarted() ) );
    connect( AudioEngine::instance(), SIGNAL( finished( Hatchet::result_ptr ) ), SLOT( audioFinished() ) );
    ...
    connect( ac->getAction( "aboutHatchet" ), SIGNAL( triggered() ), SLOT( showAboutHatchet() ) );
...
HatchetWindow::setupMenuBar()
{
    ...
    m_menuBar->setFont( HatchetUtils::systemFont() );
...
bool
HatchetWindow::eventFilter( QObject* obj, QEvent* event )
{
...
void
HatchetWindow::changeEvent( QEvent* e )
{
...
void
HatchetWindow::closeEvent( QCloseEvent* e )
{
#ifndef Q_OS_MAC
...
void
HatchetWindow::showEvent( QShowEvent* e )
{
...
void
HatchetWindow::hideEvent( QHideEvent* e )
{
...
void
HatchetWindow::keyPressEvent( QKeyEvent* e )
{
    bool accept = true;
...
void
HatchetWindow::audioStateChanged( AudioState newState, AudioState oldState )
{
    Q_UNUSED(oldState);
#ifndef Q_OS_WIN
    Q_UNUSED(newState);
    ...
    {
        play->setIcon( thumbIcon(HatchetUtils::PauseButton) );
...
void
HatchetWindow::onHistoryBackAvailable( bool avail )
{
...
void
HatchetWindow::onHistoryForwardAvailable( bool avail )
{
...
void
HatchetWindow::showSettingsDialog()
{
    if ( m_settingsDialog )
    {
...
void
HatchetWindow::showDiagnosticsDialog()
{
...
void
HatchetWindow::legalInfo()
{
    QDesktopServices::openUrl( QUrl( "http://www.hatchet-player.org/legal.html" ) );
}


void
HatchetWindow::getSupport()
{
    QDesktopServices::openUrl( QUrl( "https://hatchet.uservoice.com" ) );
}


void
HatchetWindow::reportBug()
{
    QDesktopServices::openUrl( QUrl( "https://bugs.hatchet-player.org" ) );
}


void
HatchetWindow::helpTranslate()
{
    QDesktopServices::openUrl( QUrl( "https://www.transifex.com/projects/p/hatchet/" ) );
}


void
HatchetWindow::openLogfile()
{
#ifdef WIN32
    ShellExecuteW( 0, 0, (LPCWSTR)HatchetUtils::logFilePath().utf16(), 0, 0, SW_SHOWNORMAL );
#else
    QDesktopServices::openUrl( QUrl::fromLocalFile( HatchetUtils::logFilePath() ) );
#endif
}


void
HatchetWindow::updateCollectionManually()
{
...
void
HatchetWindow::rescanCollectionManually()
{
    if ( HatchetSettings::instance()->hasScannerPaths() )
...
void
HatchetWindow::showOfflineSources()
{
    m_sourcetree->showOfflineSources(
        ActionCollection::instance()->getAction( "showOfflineSources" )->isChecked() );
    HatchetSettings::instance()->setShowOfflineSources(
        ActionCollection::instance()->getAction( "showOfflineSources" )->isChecked() );
}


void
HatchetWindow::fullScreenEntered()
{
    HatchetSettings::instance()->setFullscreenEnabled( true );
...
void
HatchetWindow::fullScreenExited()
{
    HatchetSettings::instance()->setFullscreenEnabled( false );
...
void
HatchetWindow::loadPlaylistFinished( int ret )
{
...
void
HatchetWindow::importPlaylist( const QString& url, bool autoUpdate )
{
    ...
    else if ( ext.endsWith( "jspf" ) )
    {
        ...
        connect( loader, SIGNAL( ok( Hatchet::playlist_ptr ) ), SLOT( onNewPlaylistOk( Hatchet::playlist_ptr ) ) );
        loader->load( u );
    }
    else
    {
...
void
HatchetWindow::onNewPlaylistOk( const Hatchet::playlist_ptr& pl )
{
...
void
HatchetWindow::onXSPFError( XSPFLoader::XSPFErrorCode error )
{
    QString msg;
    switch ( error )
    {
...
void
HatchetWindow::onAudioEngineError( AudioEngine::AudioErrorCode /* error */ )
{
...
HatchetWindow::createPlaylist()
{
    ...
    playlist_ptr playlist = Hatchet::Playlist::create( SourceList::instance()->getLocal(), uuid(), playlistName, info, creator, false, QList< query_ptr>() );
    ViewManager::instance()->show( playlist );
}


void
HatchetWindow::audioStarted()
{
...
void
HatchetWindow::onPlaybackLoading( const Hatchet::result_ptr result )
{
...
void
HatchetWindow::setWindowTitle( const QString& title )
{
    m_windowTitle = title;
...
void
HatchetWindow::showAboutHatchet()
{
    QString head, desc;

#ifdef QT_DEBUG
    head = tr( "<h2><b>%applicationName %1<br/>(%2)</h2>" )
         .arg( HatchetUtils::appFriendlyVersion() )
    ...
    desc = QString( "https://hatchet-player.org<br/><br/>%1<br/><br/>Christian Muehlhaeuser &lt;muesli@tomahawk-player.org&gt;<br/><br/>"
...
void
HatchetWindow::showWhatsNew_0_8()
{
...
void
HatchetWindow::checkForUpdates()
{
#ifdef Q_OS_MAC
    Hatchet::checkForUpdates();
#endif
}


void
HatchetWindow::onSearch( const QString& search )
{
    if ( !search.trimmed().isEmpty() )
    {
        if ( search.startsWith( "hatchet:" ) )
        {
            APP->loadUrl( search );
        }
        else
...
void
HatchetWindow::toggleFullscreen()
{
    tDebug() << Q_FUNC_INFO;

#if defined( Q_OS_MAC )
    Hatchet::toggleFullscreen();
#endif
}


void
HatchetWindow::crashNow()
{
    HatchetUtils::crash();
}
...
void
HatchetWindow::toggleMenuBar() //SLOT
{
#ifndef Q_OS_MAC
...
SourceTreeView*
HatchetWindow::sourceTreeView() const
{
    return m_sourcetree;
}
