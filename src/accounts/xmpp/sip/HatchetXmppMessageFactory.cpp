...
HatchetXmppMessageFactory::HatchetXmppMessageFactory()
    : m_sipInfos()
{
    m_depth = 0;
    m_state = AtNowhere;
...
QStringList HatchetXmppMessageFactory::features() const
{
    return QStringList(HATCHET_SIP_MESSAGE_NS);
}

bool HatchetXmppMessageFactory::canParse(const QStringRef &name, const QStringRef &uri, const QXmlStreamAttributes &attributes)
{
    Q_UNUSED(uri);
    Q_UNUSED(attributes);
    return name == QLatin1String("hatchet") && uri == HATCHET_SIP_MESSAGE_NS;
}

void HatchetXmppMessageFactory::handleStartElement(const QStringRef &name, const QStringRef &uri,
...
void HatchetXmppMessageFactory::handleEndElement(const QStringRef &name, const QStringRef &uri)
{
    if ( m_depth == 3 )
        m_state = AtTransport;
    else if ( m_depth == 2 )
...
void HatchetXmppMessageFactory::handleCharacterData(const QStringRef &text)
{
    /*if (m_state == AtUtc) {
...
void HatchetXmppMessageFactory::serialize(Payload *extension, QXmlStreamWriter *writer)
{
    HatchetXmppMessage *sipMessage = se_cast<HatchetXmppMessage*>(extension);

    writer->writeStartElement( QLatin1String( "hatchet" ) );
    writer->writeDefaultNamespace( HATCHET_SIP_MESSAGE_NS );
...
Payload::Ptr
HatchetXmppMessageFactory::createPayload()
{
    return Payload::Ptr( new HatchetXmppMessage( m_sipInfos ) );
}

void
HatchetXmppMessageFactory::serializeSipInfo(SipInfo &info, QXmlStreamWriter *writer)
{
    if ( info.isVisible() )
    {
