...
signals:
    void hatchetHostFound( const QString&, int, const QString&, const QString& );
...
public slots:
    void resolved( QHostInfo i )
    {
        ...
        if ( !i.hostName().isEmpty() )
            emit hatchetHostFound( ip, port, i.hostName(), nid );
        else
            emit hatchetHostFound( ip, port, "Unknown", nid );
        this->deleteLater();
    }
...
class ACCOUNTDLLEXPORT HatchetZeroconf : public QObject
{
Q_OBJECT

public:
    HatchetZeroconf( int port, QObject* parent = 0 )
...
public slots:
    void advertise()
    {
        ...
        QByteArray advert = QString( "HATCHETADVERT:%1:%2:%3" )
                               .arg( m_port )
        ...
        advert = QString( "HATCHETADVERT:%1:%2" )
                    .arg( m_port )
                    .arg( Hatchet::Database::instance()->impl()->dbid() )
                    .toLatin1();
        m_sock.writeDatagram( advert.data(), advert.size(), QHostAddress::Broadcast, ZCONF_PORT );
...
private slots:
    void readPacket()
    {
        ...
        if ( datagram.startsWith( "HATCHETADVERT:" ) &&
            Servent::isIPWhitelisted( sender ) )
        {
            ...
            if ( parts.length() == 4 )
            {
                ...
                if ( ok && Hatchet::Database::instance()->impl()->dbid() != parts.at( 2 ) )
                {
                    emit hatchetHostFound( sender.toString(), port, parts.at( 3 ), parts.at( 2 ) );
                }
            }
            else if ( parts.length() == 3 )
            {
                ...
                if ( ok && Hatchet::Database::instance()->impl()->dbid() != parts.at( 2 ) )
                {
                    ...
                    connect( n,    SIGNAL( hatchetHostFound( QString, int, QString, QString ) ),
                             this, SIGNAL( hatchetHostFound( QString, int, QString, QString ) ) );
                    n->resolve();
                }
            }
        }
